function newBingo() {
  ...
  if (
    ...
    alert(
      "Atenção, a quantidade de números do Bingo tem que ser maior que o tamanho da cartela!"
    );
  } else {
    localStorage.clear();
...
inputAmountNumbers.addEventListener("keypress", (event) => {
  if (
    event.keyCode < 48 ||
    event.keyCode > 57 ||
    inputAmountNumbers.value.length == 2
  ) {
    event.preventDefault();
  }
});
...
inputAmountCards.addEventListener("keypress", (event) => {
  if (
    event.keyCode < 48 ||
    event.keyCode > 57 ||
    inputAmountCards.value.length == 2
  ) {
    event.preventDefault();
  }
});
